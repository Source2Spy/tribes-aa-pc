...
cfx_pool *cfxpool_Expand(s32 nEntries)
{
    ...
    {
		pPool->m_CfxElements[i].m_Status = CFXSTAT_EXPIRED;
...
void cfx_Free(cfx *pCfx)
{
	...
	ASSERT(pCfx->m_Status != CFXSTAT_IDLE);
	pCfx->m_Status = CFXSTAT_IDLE;
...
cfx  *cfx_Alloc(s32 SampleId,cfx_state *pState)
{
	...
	pCfx->m_OriginalState.m_Flags = pSample->m_Flags;

    pCfx->m_OriginalState.m_CfxId = SampleId;

    pCfx->m_State       = pCfx->m_OriginalState;
    pCfx->m_Flags       = pSample->m_Flags|pState->m_Flags;
    ...
    else
    {
		...
		pCfx->m_OriginalState.m_Flags  = pState->m_Flags;
        pCfx->m_Count = 1;
...
#if 0
	Kprintf("[%8d] cfx_Alloc: Starting id %d, cfxid %d, volume %d, flags %04x\n",g_Audio.m_TimerTick,
			pState->m_Id,
			pState->m_CfxId,
			pState->m_Volume);
#endif
    ...
    if (pCfx->m_Flags & AUDFLAG_CHANNELSAVER)
    {
        ...
        while (pCfxList)
        {
            if (pCfxList->m_OriginalState.m_CfxId == pCfx->m_OriginalState.m_CfxId)
            {
                ...
                if ( ( (pCfxList->m_Status == CFXSTAT_PLAYING) || (pCfxList->m_Status == CFXSTAT_STARTING) ) &&
                     (pCfxList->m_State.m_Volume <= pCfx->m_State.m_Volume) )
                {
#if 0
					Kprintf("Channelsaver: Culled cfx #%d,cfxvol=%d,cullvol=%d\n",pCfxList->m_Identifier,pCfx->m_State.m_Volume,pCfxList->m_State.m_Volume);
#endif
#ifdef X_DEBUG
...
//    BREAK;
	...
	// A HOST_STREAM element will be explicitly killed by the hoststream code.
    if (pCfx->m_Count == 0)
    {
