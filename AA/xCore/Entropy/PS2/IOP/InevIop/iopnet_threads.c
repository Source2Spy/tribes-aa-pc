...
sceSifQueueData SendQueueData __attribute__((aligned(16)));
sceSifServeData SendServerData __attribute__((aligned(16)));
...
sceSifClientData        ReadClientData __attribute__((aligned(16)));
//-----------------------------------------------------------------------------
void NetReadThread(void)
{
    port_info               *pPort;
        ...
        do
        {
            ...
            while (pPort)
            {
                ...
                if (g_Net.ReadRequest.m_Header.Length>0)
                {
					...
					addr = InetAddressToInt(&InetAddress);
					iop_DebugMsg("iop: Received packet from %d.%d.%d.%d:%d\n",(addr & 0xff),
																			  (addr >> 8 & 0xff),
																			  (addr >> 16 & 0xff),
																			  (addr >> 24 & 0xff),
																			  InetPort);
