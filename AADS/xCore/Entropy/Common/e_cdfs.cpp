...
#include "../Auxiliary/bitmap/PNG/ZLib/zlib.h"
...
static char*                    s_PackedSignature   = "PCK0";
static u16                      s_PackBlockSize     = 0;
static pack_index_entry*        s_pPackIndexTable   = NULL;
static u32                      s_UnpackedLength    = 0;
static byte*                    s_PackedBuffer      = NULL;
static byte*                    s_UnpackedBuffer    = NULL;
static s32                      s_PackFilePointer   = 0;
static s32                      s_PackBufferIndex   = 0;
static s32                      s_PackBufferValid   = 0;
...
xbool   MyOpen(const char* pFilename)
{
    char sig[4];
    s32  BlockCount;
    if (s_pPackIndexTable)
    {
        x_free(s_pPackIndexTable);
    }
    s_pPackIndexTable = NULL;

    s_fh = fopen(pFilename,"rb");
    if (!s_fh)
    {
        return FALSE;
    }
    fread(sig,4,1,s_fh);
    if (x_memcmp(sig,s_PackedSignature,4)!=0)
    {
        fseek(s_fh,0,SEEK_SET);
        return TRUE;
    }
    fread(&s_UnpackedLength,sizeof(s32),1,s_fh);
    fread(&s_PackBlockSize,sizeof(s16),1,s_fh);
    if (s_PackBlockSize==0)
        return TRUE;
    BlockCount = (s_UnpackedLength+s_PackBlockSize-1)/s_PackBlockSize;
    s_pPackIndexTable = (pack_index_entry*)x_malloc(sizeof(pack_index_entry)*BlockCount);
    ASSERT(s_pPackIndexTable);
    fread(s_pPackIndexTable,sizeof(pack_index_entry),BlockCount,s_fh);

    s_PackBufferIndex = 0;
    s_PackBufferValid = 0;
    s_PackFilePointer = 0;
    s_PackedBuffer = (byte*)x_malloc(s_PackBlockSize);
    s_UnpackedBuffer=(byte*)x_malloc(s_PackBlockSize);
...
s32     MyRead(void* pBuffer, s32 Length)
{
    byte*   pBuff;
    s32     BlockRemain;
    s32     BlockOffset;
    ...
    if (s_pPackIndexTable)
    {
        while (Length)
        {
            BlockOffset = s_PackFilePointer % s_PackBlockSize;
            BlockRemain = s_PackBufferValid - BlockOffset;
            if (BlockRemain > Length)
                BlockRemain = Length;
            if ( ( s_PackFilePointer >= s_PackBufferIndex) && 
                 ( s_PackFilePointer < s_PackBufferIndex + s_PackBufferValid) &&
                 (  BlockRemain > 0 ) )
            {
                x_memcpy(pBuff,s_UnpackedBuffer+BlockOffset,BlockRemain);
                pBuff               += BlockRemain;
                Length              -= BlockRemain;
                LengthRead          += BlockRemain;
                s_PackFilePointer   += BlockRemain;
            }
            else
            {
                // If we are outside the buffer, read in the correct buffer.
                BlockIndex = s_PackFilePointer / s_PackBlockSize;
                fseek(s_fh,s_pPackIndexTable[BlockIndex].Offset,SEEK_SET);
                s_PackBufferValid = s_PackBlockSize;
                if (s_pPackIndexTable[BlockIndex].Size == s_PackBlockSize)
                {
                    fread(s_UnpackedBuffer,s_pPackIndexTable[BlockIndex].Size,1,s_fh);
                    s_PackBufferValid = s_PackBlockSize;
                }
                else
                {
                    fread(s_PackedBuffer,s_pPackIndexTable[BlockIndex].Size,1,s_fh);
                    Status = uncompress(s_UnpackedBuffer,(unsigned long*)&s_PackBufferValid,s_PackedBuffer,s_pPackIndexTable[BlockIndex].Size);
                    ASSERT(Status == 0);
                }
                s_PackBufferIndex = BlockIndex * s_PackBlockSize;
            }
        }
        return LengthRead;
    }
    ...
    {
        return fread(pBuffer,1,Length,s_fh);
    }
}

void    MySeek(s32 Position)
{
    if (s_pPackIndexTable)
    {
        s_PackFilePointer = Position;
    }
    else
    {
        fseek(s_fh,Position,SEEK_SET);
    }
}

u32     MyFTell(void)
{
    return s_PackFilePointer;
}
...
#else
        s32 BytesRead = MyRead( &s_Header, sizeof(s_Header));
#endif
...
#else
                    s32 BytesRead = MyRead( s_pFileTable, s_Header.FileTableLength);
#endif
...
#else
                    s32 BytesRead = MyRead( s_pStringTable, s_Header.StringTableLength);
#endif
    ...
    while( BytesLeft > 0 )
    {
        ...
        else
        {
            ...
            if( (s_Header.FirstSectorOffset + SectorByte) != s_cdfsPosition )
            {
                ...
                ASSERT(pos>=0);
#else
                MySeek(s_Header.FirstSectorOffset + (s32)SectorByte);
#endif
...
#else
            s32 CacheBytesRead = MyRead( s_pCacheData, BytesToCache);
#endif
