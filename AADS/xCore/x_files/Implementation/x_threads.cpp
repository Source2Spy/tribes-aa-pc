...
xthread::xthread(s32 StackSize)
{
    ...
    sem_init(&m_SuspendSemaphore,0,0);
	m_SysThreadId = pthread_self();
	Link();
#else
...
xthread::xthread    (entry_fn *pEntry,const char *pName,s32 StackSize,s32 Priority)
{
	...
	Link();
    sem_init(&m_SuspendSemaphore,0,0);
	error = pthread_create((pthread_t*)&m_SysThreadId,NULL,(entry_fn*)pEntry,NULL);
...
#elif defined(TARGET_LINUX)
    (void)Flags;
    ASSERTS(pthread_self() == (pthread_t)m_SysThreadId,"Trying to suspend another thread other than self");

    sem_wait(&m_SuspendSemaphore);
#else
...
#elif defined(TARGET_LINUX)
    (void)Flags;
    ASSERTS(pthread_self() == (pthread_t)m_SysThreadId,"Trying to resume myself!");
    sem_post(&m_SuspendSemaphore);
#else
