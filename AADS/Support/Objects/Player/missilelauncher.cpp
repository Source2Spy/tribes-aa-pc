         ...
         ( GetWeaponAmmoCount(m_WeaponCurrentType) >= m_WeaponInfo->AmmoNeededToAllowFire) )
    {
        ...
        while( (pObject = ObjMgr.GetNextSelected()) )
        {
            ...
            if( pObject->GetAttrBits() & ATTR_VEHICLE )
            {
                ...
                {
                    // Allied vehicle must contain an enemy player.
                    vehicle* pVehicle = (vehicle*)pObject;
                    if( !pVehicle->HasEnemyOccupant( m_TeamBits ) )
                        continue;
                }
            }
            else
            {
                // Non vehicle must be an enemy object.
            ...
            s_Collider.RaySetup( this, ViewPos, BBPos, m_ObjectID.GetAsS32(), TRUE );
            s_Collider.Exclude( pObject->GetObjectID().GetAsS32() );
