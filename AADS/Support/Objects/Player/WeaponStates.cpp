...
void player_object::Weapon_CheckForUpdatingShape()
{
    ...
    if ( (m_WeaponRequestedType != m_WeaponCurrentType) || (!m_WeaponInstance.GetShape()) )
    {
        ...
        m_WeaponFireDelay[ m_WeaponCurrentSlot ] = MAX( NEW_WEAPON_FIRE_DELAY, 
                                                        m_WeaponFireDelay[ m_WeaponCurrentSlot ] ) ;
...
void player_object::Weapon_AdvanceState(f32 DeltaTime)
{
    ...
    for( s32 i = 0; i < 6; i++ )
    if (m_WeaponFireDelay[ i ] > 0)
    {
        m_WeaponFireDelay[ i ] -= DeltaTime ;
        if (m_WeaponFireDelay[ i ] < 0)
            m_WeaponFireDelay[ i ] = 0 ;
    }
...
void player_object::Weapon_Setup_FIRE()
{
    ...
    // Update fire delay
    m_WeaponFireDelay[ m_WeaponCurrentSlot ] += m_WeaponInfo->FireDelay ;
